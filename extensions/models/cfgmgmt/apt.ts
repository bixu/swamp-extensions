...
async function queryPackages(client, packages, g) {
  ...
  for (const pkg of packages) {
    const r = await exec(client, wrapSudo(`dpkg-query -W -f '\${Status} \${Version}' ${JSON.stringify(pkg)} 2>/dev/null`, so));
  ...
  methods: {
      ...
      execute: async (_args, context) => {
        ...
        try {
          ...
          if (toInstall.length > 0) {
            const r = await exec(client, wrapSudo(`DEBIAN_FRONTEND=noninteractive apt-get install -y ${toInstall.join(" ")}`, so));
            stdout += r.stdout;
            stderr += r.stderr;
            if (r.exitCode !== 0) {
              const handle = await context.writeResource("state", g.nodeHost, {
          ...
          if (toRemove.length > 0) {
            const r = await exec(client, wrapSudo(`DEBIAN_FRONTEND=noninteractive apt-get remove -y ${toRemove.join(" ")}`, so));
            stdout += r.stdout;
            stderr += r.stderr;
            if (r.exitCode !== 0) {
              const handle = await context.writeResource("state", g.nodeHost, {
      ...
      execute: async (_args, context) => {
        ...
        try {
          ...
          const handle = await context.writeResource("state", g.nodeHost, {
            ...
            stdout: r.stdout,
            stderr: r.stderr,
            error: r.exitCode !== 0 ? `apt-get update failed with exit code ${r.exitCode}` : null,
            timestamp: new Date().toISOString(),
          });
          return { dataHandles: [handle] };
        } catch (err) {
    ...
    upgrade: {
      ...
      execute: async (_args, context) => {
        ...
        try {
          const client = await connect(g);
          const r = await exec(client, wrapSudo("DEBIAN_FRONTEND=noninteractive apt-get upgrade -y", sudoOpts(g)));
          const handle = await context.writeResource("state", g.nodeHost, {
            status: r.exitCode === 0 ? "applied" : "failed",
            packages: null,
            ...
            stdout: r.stdout,
            stderr: r.stderr,
            error: r.exitCode !== 0 ? `apt-get upgrade failed with exit code ${r.exitCode}` : null,
            timestamp: new Date().toISOString(),
          });
          return { dataHandles: [handle] };
        } catch (err) {
    ...
    list: {
      ...
      execute: async (_args, context) => {
        ...
        try {
          const client = await connect(g);
          const r = await exec(client, wrapSudo("dpkg-query -W -f '${Package} ${Version}\\n'", sudoOpts(g)));
          if (r.exitCode !== 0) {
            const handle = await context.writeResource("state", g.nodeHost, {
              status: "failed",
              packages: null,
