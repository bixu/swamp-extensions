...
const GlobalArgsSchema = z.object({
  ...
  ensure: z.enum(["present", "absent"]).describe("Whether file should be present or absent"),
...
const StateSchema = z.object({
  ...
  current: z.object({
    ...
    contentSha256: z.string().nullable().describe("SHA-256 hash of current content"),
...
async function gather(client, path, g) {
  ...
  if (line === "NOTFOUND") {
    return { exists: false, isFile: false, owner: null, group: null, mode: null, contentSha256: null };
  }
  const [fileType, owner, group, mode] = line.split("|");
  const isFile = fileType === "regular file" || fileType === "regular empty file";

  let contentSha256 = null;
  if (isFile) {
    const hashResult = await exec(client, wrapSudo(`sha256sum ${JSON.stringify(path)} 2>/dev/null | awk '{print $1}'`, so));
    contentSha256 = hashResult.stdout.trim() || null;
  }

  return { exists: true, isFile, owner, group, mode: `0${mode}`, contentSha256 };
}
    ...
    check: {
      ...
      execute: async (_args, context) => {
        ...
        try {
          ...
          if (g.ensure === "present" && g.content !== undefined && current.isFile && current.contentSha256) {
          ...
          const handle = await context.writeResource("state", g.nodeHost, {
            ...
            status: "failed",
            current: { exists: false, isFile: false, owner: null, group: null, mode: null, contentSha256: null },
            changes: [],
            error: err.message,
            timestamp: new Date().toISOString(),
          });
