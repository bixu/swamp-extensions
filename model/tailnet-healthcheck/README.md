...
Tailnet health reporting for [swamp](https://github.com/systeminit/swamp) —
queries the Tailscale API to find devices running outdated client versions and
optionally posts a report to Slack.
...
The security floor (minimum safe client version) is determined dynamically from
the
[Tailscale security bulletins RSS feed](https://tailscale.com/security-bulletins/index.xml),
falling back to a hardcoded constant if the feed is unavailable.
